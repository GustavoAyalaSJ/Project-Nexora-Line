(function () {
    // Move inline script from index6.html into React entry (runs before React auth effect).
    fetch('./.company_secret')
        .then((resp) => {
        ...
        .then((text) => {
            ...
            if (m) window.COMPANY_SECTION_PASSWORD = m[1].trim();
        })
        .catch(() => {
            // silently ignore (keeps existing auth behavior)
        });
})();
const ESTADOS_DATA = [
        ...
        <React.Fragment>
            ...
            {dropdownAberto === 'profile-button' && (
                <div ref={dropdownRef} className="dropdown-content" id="profile-dropdown">
                    ...
                    <a href="IntroducedPage.html" className="settings-link"><i className="bi bi-box-arrow-left"></i> Sair</a>
                    <button onClick={() => setDropdownAberto(null)} className="close-dropdown">Fechar</button>
                </div>
            )}
        </React.Fragment>
